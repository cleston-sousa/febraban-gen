...
public class EstadosController {
	...
	public EstadoResponse adicionar(@RequestBody EstadoRequest estadoRequest) {
		var add = ModelMapperUtils.to(estadoRequest, AdicionarEstadoDTO.class);
	...
	public EstadoResponse alterar(@PathVariable Integer estadoId, @RequestBody EstadoRequest estadoRequest) {
		var edit = ModelMapperUtils.to(estadoRequest, AlterarEstadoDTO.class);
		var resultado = this.estadosService.salvar(estadoId, edit);
		return ModelMapperUtils.to(resultado, EstadoResponse.class);
	}

	@GetMapping("/codigo/{estadoCodigo}")
	public EstadoResponse obterPorCodigo(@PathVariable String estadoCodigo) {
		var resultado = this.estadosService.obterPorCodigo(estadoCodigo);
		return ModelMapperUtils.to(resultado, EstadoResponse.class);
	}

	@GetMapping("/nome/{estadoNome}")
	public List<EstadoResponse> obterPorNome(@PathVariable String estadoNome) {
		var resultado = this.estadosService.obterContendoNome(estadoNome);
		return ModelMapperUtils.toList(resultado, EstadoResponse.class);
	}
